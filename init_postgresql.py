        ...
        try:
            ...
            if not Material.query.first():
                materials = [
                    ...
                    Material(name="銅", description="導電性良好的金屬", base_value=150.0, rarity="uncommon"),
                    Material(name="石", description="基礎建築材料", base_value=50.0, rarity="rare")
                ]
                db.session.add_all(materials)
                db.session.commit()
                print("✅ 創建默認材料完成")
            
            print("🎉 PostgreSQL 數據庫初始化完成！")
            
        except Exception as e:
            print(f"❌ 數據庫初始化失敗: {str(e)}")
            print("請檢查 PostgreSQL 連接設置和數據庫是否存在")
